...
function makeSeries(profile: Profile): SeriesResponse {
  ...
  const points: SeriesPointResponse[] = buckets.map((bucket, index) => {
    ...
    return {
      ...
      speed_upload_sample_sum: bytes(BigInt(uploadRate) * BigInt(samples)),
      speed_download_sample_sum: bytes(BigInt(downloadRate) * BigInt(samples)),
      speed_sample_count: samples,
